...
### 3.1 姿态向量插值
我们定义起始姿态角度为 $P_{\text{start}}$，目标姿态为 $P_{\text{end}}$。引入时间系数 $t \in [0, 1]$，当前帧的姿态 $P_{\text{curr}}$ 为：

$$\vec{P}_{\text{curr}}(t) = (1 - t) \cdot \vec{P}_{\text{start}} + t \cdot \vec{P}_{\text{end}}$$

### 3.2 运动平滑化
为了让动画更专业，我利用正弦函数模拟了 **Ease-in-Out** 效果，使方块在两端有轻微的减速停顿感：
...
**Git的折腾记录**：虽然处理SSH密钥很痛苦，但现在能熟练地使用git push同步我的图形学进展，成就感爆棚。
